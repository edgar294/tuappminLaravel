    ...
    public function login(Request $request)
    {
        ...
        try {
            if($request->acceso != 'appleID'){
                if (! $token = JWTAuth::attempt(['email' => $request->email, 'password' => $request->password, 'rol_id' => [4], 'status' => 'Activo'])) {
                    return response()->json(['error' => 'Credenciales incorrectas'], 200);
                }
            }
            else{
                if (! $token = JWTAuth::attempt(['user' => $request->email, 'password' => $request->password, 'rol_id' => [4], 'status' => 'Activo'])) {
                    return response()->json(['error' => 'Credenciales incorrectas'], 200);
                }
            }            
